...
A production-ready, fully bilingual (Arabic & English) e-commerce/catalog website for **VilmoraTex Trading & Industry**, built with Next.js 16 (App Router), TypeScript, and Tailwind CSS 4. Every page is statically prerendered in both languages at build time, and the site deploys to **Cloudflare Workers** (via the [OpenNext Cloudflare adapter](https://opennext.js.org/cloudflare)) directly from **GitHub**.
...
- **@opennextjs/cloudflare** + **Wrangler** — deploys the app to Cloudflare Workers (server-rendering, not a static file host)
...
`npm run dev` uses the regular Next.js dev server (fastest reload loop). To test the app the way it will actually run in production — inside the Cloudflare Workers runtime (`workerd`), not Node.js — use:

```bash
npm run preview   # builds with OpenNext, then serves it locally via `wrangler dev`
```

Deploy straight to Cloudflare:

```bash
npm run deploy    # builds with OpenNext, then runs `wrangler deploy`
```
...
`npm run deploy` (and `npm run preview`) run `opennextjs-cloudflare build`, which transforms the standard Next.js build into a Cloudflare Worker bundle in **`.open-next/`** (`worker.js` + static `assets/`) — this is what actually runs on Cloudflare, not a plain static folder.

## Bilingual / i18n Architecture

The site supports **English (`/en`)** and **Arabic (`/ar`)**, with every page prerendered at build time in both languages, and no middleware (the routing/redirect/locale-detection scheme is deliberately middleware-free, which keeps it simple and portable even though Cloudflare Workers does support middleware).
...
Locally, Next.js reads them from `.env.local` as usual. For deployment on Cloudflare, set them as **Build variables** in your Workers Builds project settings (Cloudflare dashboard → your Worker → **Settings → Build → Variables**) — see [Deployment to Cloudflare Workers](#deployment-to-cloudflare-workers) below. If you deploy via the GitHub Actions workflow instead, set them as **repository variables** (`Settings → Secrets and variables → Actions → Variables`).

## Project Structure

```
wrangler.jsonc           Cloudflare Worker config: entry point, assets binding, compatibility flags
open-next.config.ts      OpenNext adapter config (caching, etc.)
app/
...
After `npm run build` / `npm run preview` / `npm run deploy`, an additional **`.open-next/`** directory appears (git-ignored) — this is the actual Cloudflare Worker bundle (`worker.js` + prerendered `assets/`) that OpenNext generates from the standard Next.js build.
...
Supported formats: WebP, JPG, PNG, SVG, GIF, AVIF for images; MP4, WebM, MOV for video; PDF/DOC/XLS for documents. All images use `next/image` with `images.unoptimized: true` and lazy loading by default — this avoids requiring a paid Cloudflare Images subscription; see the note in "Deployment" if you want real Cloudflare Images optimization instead.
...
3. Because `next.config.ts` sets `images.unoptimized: true`, any local path or external URL works without additional Cloudflare-side image optimization configuration. (OpenNext Cloudflare does support wiring `next/image` up to real Cloudflare Images optimization if you want it later — see the OpenNext Cloudflare docs.)
...
This site runs on **Cloudflare Workers** (server-rendered by a Worker, not hosted as static files) via the [OpenNext Cloudflare adapter](https://opennext.js.org/cloudflare). This is a deliberate change from a plain static export — it's what lets every page keep working correctly (SSR, dynamic routing, i18n redirects) with zero middleware limitations, while still prerendering every page at build time for speed.

> **Important — "Workers", not "Pages":** Cloudflare's dashboard has two similarly named products: **Pages** (for static sites / static-export frameworks) and **Workers** (for apps with a server, including OpenNext-adapted Next.js apps like this one). If you previously created a **Pages** project for this repo, that won't work anymore — create a **Workers** project instead (Option A below). This is almost certainly the fix if you were seeing build errors trying to deploy this project through a Pages project.

### Option A — Cloudflare Workers Builds (Git integration, simplest, no secrets needed)

1. Push this repository to GitHub.
2. In the Cloudflare dashboard, go to **Compute (Workers) → Create → Import a repository**, and select this repository. (If you already have a **Pages** project pointed at this repo, delete/disconnect it first, or just create a fresh Workers project — don't reuse the Pages one.)
3. Cloudflare will detect the `wrangler.jsonc` file and the Next.js/OpenNext setup automatically and pre-fill the build/deploy command (`npm run deploy`, i.e. `opennextjs-cloudflare build && opennextjs-cloudflare deploy`). Leave that as-is.
4. Under **Settings → Build → Variables**, add `NEXT_PUBLIC_SITE_URL`, `NEXT_PUBLIC_WHATSAPP_NUMBER`, `NEXT_PUBLIC_CONTACT_EMAIL`, `NEXT_PUBLIC_QUOTE_FORM_ENDPOINT` as needed (see table above) — these need to be set here so they're available to `next build` during the CI build, not just at runtime.
5. Deploy — Cloudflare will rebuild and redeploy automatically on every push to your chosen branch.

### Option B — GitHub Actions (`.github/workflows/deploy.yml`)

This repo includes a ready-made workflow that builds the app with OpenNext and deploys it via `wrangler deploy` (using `cloudflare/wrangler-action`). To use it:

1. In your GitHub repo settings, add these **secrets**: `CLOUDFLARE_API_TOKEN` (a token with the **Workers Scripts: Edit** permission) and `CLOUDFLARE_ACCOUNT_ID`.
2. Add any of the environment variables from the table above as **repository variables** (`Settings → Secrets and variables → Actions → Variables`) if you want to override the defaults.
3. Push to `main` — the workflow builds and deploys automatically to the Worker named in `wrangler.jsonc` (`"name": "vilmoratex"`).

### Option C — Deploy from your own machine

```bash
npx wrangler login   # one-time: authenticates the Wrangler CLI with your Cloudflare account
npm run deploy        # builds with OpenNext, then deploys to Cloudflare Workers
```

### Custom domain

Once deployed, attach your domain in the Cloudflare dashboard under your Worker's **Settings → Domains & Routes → Add → Custom domain**, and update `NEXT_PUBLIC_SITE_URL` to match.
