...
export default function AddToCartControls({ product }: { product: Product }) {
  ...
  const inquiryHref = buildWhatsAppLink(
    buildProductInquiryMessage(product.name, typeof window !== "undefined" ? window.location.href : `/shop/${product.slug}`)
  );

  return (
    <div className="space-y-5">
      <div>
        <p className="mb-2 text-sm font-semibold text-brand-charcoal">Size</p>
            ...
            {product.colors.map((c) => (
              <button
                ...
                className={`h-8 w-8 rounded-full border-2 ${
                  color === c.name ? "border-brand-plum" : "border-transparent"
                }`}
      ...
      <div>
        <p className="mb-2 text-sm font-semibold text-brand-charcoal">Quantity</p>
        ...
        {product.minOrderQty && (
          <p className="mt-1 text-xs text-brand-charcoal/50">
            Wholesale minimum order: {product.minOrderQty}+ pieces per style
          </p>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-3 pt-2">
        ...
        >
          <svg viewBox="0 0 24 24" className="h-4 w-4" fill={has(product.slug) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
